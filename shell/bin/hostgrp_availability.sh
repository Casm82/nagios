...
for hostgrp in `cat $BASE_DIR/src_hostgroups`; do
 ...
 "http://nagios.kupol.ru/nagios/cgi-bin/avail.cgi?show_log_entries=&hostgroup=$hostgrp&timeperiod=custom&smon=$SMON&sday=$SDAY&syear=$SYEAR&shour=$SHOUR&smin=$SMIN&ssec=$SSEC&emon=$EMON&eday=$EDAY&eyear=$EYEAR&ehour=$EHOUR&emin=$EMIN&esec=$ESEC&rpttimeperiod=$TIMEPERIOD" 
